...
%   cfg_run = make_delivery_config("time_diversity_4db_followup");
%   cfg_run = make_delivery_config("time_diversity_fractional_gabp_exploration");
%   cfg_run = make_delivery_config("time_diversity_lch6_tau48_followup");
%   cfg_run = make_delivery_config("time_diversity_tau48_sixline");
%
% 交付版主路线:
...
switch mode
    ...
    case {"time_diversity_smoke", "time_diversity_pilot", ...
        ...
        cfg_run.time_diversity = struct( ...
            ...
            'spatial_pairs', {{'wdm', 'dft', 'svd'}}, ...
            'Lch_values', [4, 6], ...
            'SNR_dB_list', [8, 10, 12, 14, 17, 20, 23], ...
            'target_errors', 100, ...
            'min_frames', 10, ...
            'max_frames', 1500, ...
        ...
        elseif mode == "time_diversity_4db_followup"
            % Single-point follow-up using the exact v6 detector/fairness
            % contract. The wider detector iteration allowance is diagnostic;
            % this comparable Online point remains locked to 20 iterations.
            cfg_run.time_diversity.SNR_dB_list = 4;
            cfg_run.time_diversity.spatial_pairs = {'wdm'};
            cfg_run.time_diversity.Lch_values = 6;
            cfg_run.time_diversity.max_frames = 150;
            cfg_run.time_diversity.siso_frames = 1;
            cfg_run.time_diversity.siso_SNR_dB_list = 4;
            cfg_run.time_diversity.enable_escalation = false;
        elseif mode == "time_diversity_fractional_gabp_exploration"
            ...
            cfg_run.time_diversity.siso_SNR_dB_list = 0;
            cfg_run.time_diversity.enable_escalation = false;
            cfg_run.time_diversity.explicit_stages = struct( ...
                'name', {'anchor_lch6_kmax2_tau32', ...
                         'lch8_kmax2_tau32', ...
                         'lch8_kmax3_tau32', ...
                         'lch8_kmax3_tau48'}, ...
                'Lch', {6, 8, 8, 8}, ...
                'v_max_kmh', {860, 860, 1100, 1100}, ...
                'tau_max_us', {32, 32, 32, 48}, ...
                'single_variable_change', { ...
                    'none; comparable Lch=6 anchor', ...
                    'Lch:6->8', ...
                    'v_max_kmh:860->1100 (kmax:2->3)', ...
                    'tau_max_us:32->48'});
        elseif mode == "time_diversity_lch6_tau48_followup"
            % Single-variable follow-up to the v9 Lch6/kmax2/tau32 anchor.
            % Keep path count and Doppler fixed; only delay support changes.
            cfg_run.tau_max_us = 48;
            cfg_run.time_diversity.SNR_dB_list = [-8, -6, -4, -2, 0, 1, 2, 4];
            cfg_run.time_diversity.spatial_pairs = {'wdm'};
            cfg_run.time_diversity.doppler_modes = {'fractional'};
            cfg_run.time_diversity.detectors = {'gabp'};
            cfg_run.time_diversity.Lch_values = 6;
            cfg_run.time_diversity.max_frames = 1000;
            cfg_run.time_diversity.gabp.max_iterations = 40;
            cfg_run.time_diversity.siso_frames = 1;
            cfg_run.time_diversity.siso_SNR_dB_list = 0;
            cfg_run.time_diversity.enable_escalation = false;
            cfg_run.time_diversity.explicit_stages = struct( ...
                'name', 'lch6_kmax2_tau48', ...
                'Lch', 6, ...
                'v_max_kmh', 860, ...
                'tau_max_us', 48, ...
                'single_variable_change', ...
                    'tau_max_us:32->48 relative to v9 anchor');
        elseif mode == "time_diversity_tau48_sixline"
            % Six-line precoding extension of the audited v10 contract.
            % Remove only 1 dB and run WDM/DFT/SVD under the same paired
            % channel, bits/noise, GaBP, and adaptive stopping settings.
            cfg_run.tau_max_us = 48;
            cfg_run.time_diversity.SNR_dB_list = [-8, -6, -4, -2, 0, 2, 4];
            cfg_run.time_diversity.spatial_pairs = {'wdm', 'dft', 'svd'};
            cfg_run.time_diversity.doppler_modes = {'fractional'};
            cfg_run.time_diversity.detectors = {'gabp'};
            cfg_run.time_diversity.Lch_values = 6;
            cfg_run.time_diversity.max_frames = 1000;
            ...
            cfg_run.time_diversity.enable_escalation = false;
            cfg_run.time_diversity.explicit_stages = struct( ...
                'name', 'lch6_kmax2_tau48_sixline', ...
                'Lch', 6, ...
                'v_max_kmh', 860, ...
                'tau_max_us', 48, ...
                'single_variable_change', ...
                    'v10 tau48 contract plus DFT/SVD spatial pairs');
        end
    ...
    case {"paperfig", "paperfig_iso", "paperfig_vmf", "paperfig_capacity", "paperfig_low_mimo"}
        ...
        cfg_run.csi_case_labels = fixed_var_5e4_csi_labels();
        cfg_run.strategies = {'full'};
    ...
    case "iso_gabp_adaptive"
        % Latest strict-isotropic 8x8/Ns=60 figure, GaBP detector only.
        % Each scheme x CSI curve stops independently after at least 5
        % frames once 100 errors are observed, with a hard cap of 200 frames.
        reference_stream_count = 60;
        average_reference_SNR_dB_list = 0:5:20;
        cfg_run.numFrames_BER = 200;
        cfg_run.SNR_dB_list = average_reference_SNR_dB_list - ...
            10 * log10(reference_stream_count);
        cfg_run.kappa_list = [0, 0.0005];
        cfg_run.csi_error_mode = 'fixed_var';
        cfg_run.csi_case_labels = fixed_var_5e4_csi_labels();
        cfg_run.strategies = {'full'};
        cfg_run.plot_csi_cases_together = true;
        cfg_run.ber_scenarios = struct( ...
            'label', {'strict_isotropic'}, ...
            'pas_model', {'isotropic'}, ...
            'cv', {1.0}, ...
            'use_perpath_sigma', {false});
        cfg_run.quick_stream_cap = [];
        cfg_run.run_capacity = false;
        cfg_run.run_low_mimo_precoding = false;
        cfg_run.iso_gabp = struct( ...
            'runner_version', 'iso-gabp-adaptive-20260723.3', ...
            'scientific_label', 'candidate', ...
            'reference_stream_count', reference_stream_count, ...
            'average_reference_SNR_dB_list', ...
                average_reference_SNR_dB_list, ...
            'average_reference_snr_definition', ...
                'N_s_times_Es_stream_over_N0', ...
            'assets', struct( ...
                'active_run_id_file', '_ACTIVE_ISO_GABP_SWEEP_V3_ID.txt', ...
                'result_mat', 'iso_gabp_adaptive_results.mat', ...
                'summary_csv', 'iso_gabp_adaptive_summary.csv', ...
                'figure_png', 'ber_strict_isotropic_gabp_only.png'), ...
            'stop', struct( ...
                'min_frames', 5, ...
                'target_errors', 100, ...
                'max_frames', 200), ...
    ...
    otherwise
        error('make_delivery_config:unknownMode', ...
            ['Unknown mode "%s". Use quick, smoke, fullmini, local2min, paper, ' ...
             ...
             'time_diversity_lch6_tau48_followup, ' ...
             'time_diversity_tau48_sixline, ' ...
             'iso_gabp_adaptive, or time_diversity_online.'], mode);
end
...
function labels = fixed_var_5e4_csi_labels()
labels = {'perfect CSI', 'fixed-var CSI, sigma_e^2=5e-4'};
end
