...
if strcmp(cfg_run.iso_gabp.scientific_label, 'candidate') && N_s ~= 60
    error('run_online_iso_gabp_sweep:streamCount', ...
        'Frozen scientific contract requires N_s=60, got %d.', N_s);
end
...
fprintf(' run_id=%s | 8x8 | N_s=%d | SNR=[%s] dB\n', ...
    run_id, N_s, num2str(cfg_run.SNR_dB_list));
...
for i_snr = 1:n_snr
    ...
    for frame_index = next_frame:cfg_run.iso_gabp.stop.max_frames
        ...
        for i_csi = 1:n_csi
            ...
            H_hat = inject_csi_error(H_phys, csi_value, 10^(snr_db / 10), ...
            ...
            for i_scheme = 1:n_scheme
                ...
                frame = simulate_imperfect_csi_gabp_frame( ...
                    ...
                    cfg_run.QAM_order, snr_db, frame_opts);
    ...
    if ~all_curves_stopped(states)
        error('run_online_iso_gabp_sweep:incompleteSNR', ...
            'SNR=%g dB reached the frame loop without closing all curves.', snr_db);
    end
...
function save_checkpoint_atomic(file, run_contract, states, next_frame, snr_db)
temporary = [file '.tmp.mat'];
save(temporary, 'run_contract', 'states', 'next_frame', 'snr_db', '-v7.3');
...
package.SNR_dB = cfg.SNR_dB_list;
...
package.snr_definition = cfg.snr_definition;
...
n_rows = numel(package.SNR_dB) * numel(package.csi_case_labels) * ...
    numel(package.schemes);
rows = cell(n_rows, 11);
row_index = 0;
for i_snr = 1:numel(package.SNR_dB)
...
summary = cell2table(rows, 'VariableNames', { ...
    'scheme', 'csi_case', 'snr_db', 'frames', 'errors', 'bits', ...
