...
for iDoppler = 1:numel(td.doppler_modes)
    ...
    [cfg_afwdm, cfg_ofwdm, waveform_audit] = ...
        make_time_diversity_waveform_pair(cfg_mode, Us_wdm, Ur_wdm, td.N_s, cfg_run);
    ...
    for iDetector = 1:numel(td.detectors)
        ...
        for iSNR = 1:numel(td.SNR_dB_list)
            ...
            for frame = 1:td.max_frames
                ...
                H_phys = build_delivery_channel_taps(scenario, seed);
            ...
            stats_opts = struct( ...
                ...
                'bootstrap_seed', td.bootstrap_seed + 1000 * iDoppler + 100 * iDetector + iSNR, ...
                'confidence', 0.95);
